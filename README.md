...
Damophus 是一个自用的思源增强插件，集中实现现有工具没有同时覆盖、或难以直接组合的个人工作流。目前主要包含四部分：让 Markdown 题源进入扫描、索引、练习与恢复流程；在题目与考点笔记之间提供虚拟关系栏；管理思源 AI 技能并保持其与本地源同步；以及改善移动端标题栏、面包屑和顶部玻璃表面的可用性。
...
按模块分组的完整功能清单。🆕 标记 0.7.0 新增能力，其余为既有能力。日常使用频率高的小功能在前，重型题库见文末。
...
- “各种块混搭列表”把非列表块变为一级列表项，并把紧随其后的原列表整体作为子列表；选择需从非列表块开始
- 混合类型可统一为有序/无序；有序编号自动重排；单步撤销/重做；四种命令可绑定快捷键
- 列表项可从块菜单一键移出为独立段落，保留原有正文、Callout 与列表顺序
...
### 快捷操作（lets-layout-actions）

- 为每个入口选择思源系统命令、插件命令或编辑器命令，显示在 Damophus 菜单或可选 Dock 中
- 一键切换左侧、右侧、底部面板布局；题库入口复用同一套命令、菜单与跨端 Dock 注册接口
- 子模块停用后自动隐藏入口，移动端插件栏提供原生外观快捷入口
- 命令名优先显示本地化名称（如 Damophus 命令显示中文名），编辑器命令所属分类按思源界面语言显示

### 技能管理（lets-skill-manager）

- 管理思源 AI 使用的技能：查看、新建、编辑、重命名、删除与同步，支持搜索、状态筛选和排序
- 只读预览走思源原生 Lute 渲染；按完整技能目录指纹检测更新，支持启动检测、启动同步和可复制的过程日志
...
### 文档历史差异（lets-document-history-diff）

- 按时间选择历史版本，以统一或并排视图对比当前文档内容；深浅模式同步，可拖动或键盘调整历史列表宽度
- 单块菜单可直接查看该块相邻历史版本的 Kramdown 差异

### 块全宽显示（lets-block-fullwidth） 🆕

- 在块标菜单中为图片、表格、数据库、iframe、横向超级块单独开启全宽，让宽内容撑满编辑区
- 文档标题菜单可按类型一键配置整篇文档，属性与 Asri 主题的「全宽显示」互通，不受当前主题限制
- 图片段落可额外选择「突破容器全宽」，让列表、引述、超级块里的图片也撑满编辑区
- Damophus 菜单内置「全部文档默认全宽」开关，免逐文档设置，随时开关，单块仍可排除

### 表格自适应（lets-table-fit）

- 文档表格根据屏幕宽度自动调整，长内容改为换行显示，不再需要横向滚动
- 可从 Damophus 菜单即时开关，与设置页保持一致

### 相同文字格式刷（lets-style-brush）

- 调用已安装的“格式刷”插件复制行内格式，批量应用到当前文档或当前标题下的相同文字
- 使用独立的文字选择图标与原格式刷区分；可完整识别当前标题的文字范围
- 编辑器工具栏按钮已同步注册到思源快捷键设置，可直接为它绑定键盘快捷键

### Callout 外观（lets-callout-appearance）

- 实时预览的 Callout 外观：类型配色、内容间距、引述与嵌入块边框、题库标记可分别开关
- 使用 Callout 增强时保留编辑、折叠、类型菜单与自定义图标，并约束嵌套 Callout 不超出父级

### Callout 工具（lets-callout-tools）

- 智能插入：在已有内容前插入 Callout 时自动将该内容纳入其中；文档末尾创建时立即提供可输入的空白正文
- 块菜单可批量转换五种 Callout 类型；多个块转换时可选择以第一个标题作为 Callout 标题并保留行内格式

### 移动端斜杠菜单（lets-mobile-slash-menu）

- 移动端网页直接输入 `/` 即可呼出并筛选思源原生斜杠菜单，可在模块设置中随时关闭
- 兼容思源异步渲染的斜杠菜单，打开后立即以增强网格布局呈现并即时筛选

### 习题专注（lets-exercise-focus）

- 根据引述块内的标题文本与级别识别习题，保留标题与指定块类型，其余内容模糊并在悬停时显示
- 切换命令可绑定到布局快捷操作 Dock；保留的块类型可复用块类型多选器配置
- 编辑过程中已揭晓或正在编辑的遮罩状态保持不重置

### 代码片段工作台（lets-snippet-audit）

- 自动识别并预览现有代码片段样式，支持搜索、启停、新建、编辑和删除片段
- 可从插件菜单或独立标签页打开；设置页中的样式类设置项共享同一实时预览组件

### 展开插件菜单（lets-expanded-plugin-menu） 🆕

- 把“插件”菜单及其嵌套命令展开为一个自适应多列面板，并自动发现其真实一级入口生成专用展开开关
- 保留外部插件、声明路径与旧文字规则的高级兼容入口
- 面板内插件入口支持拖拽排序，松开即保存顺序

### 设置与入口管理 🆕

- 设置总览新增自适应布局开关，自动填补高低卡片之间的空隙并记住用户选择
- Damophus 菜单模块顺序支持拖拽自定义，拖拽时显示明确的放置位置
- 入口管理按模块分组为紧凑视图，显示递归声明中的叶子开关；可控制模块详情页是否显示精简的启用开关
- 新装默认关闭全部子模块开关，用户可按需在设置中逐个开启；已保存的用户设置保持不变

### 考点关系（lets-topic-relations）

- 题目块与考点笔记块下方虚拟展示考点笔记和关联题目，不写入任何块
- 紧凑考点、主来源与数量、展开分组三种显示模式；思源原生悬浮预览与移动端底部面板
- 笔记来源优先级与关系栏自定义样式可配置；隐藏自身关系，关系标记保持在编辑器块外
...
- 不定项模式：练习时隐藏单选、多选提示，原单选题也可以选择多个选项；答题页“更多”菜单可实时开关并记住最新选择
...
- **关系视图不落盘**：题目与考点笔记的关系在块下方虚拟展示，不写入任何块，隐藏自身关系并保持标记在编辑器块外。
- **技能保持真实文件**：技能管理默认经 ChezMoi 把本地更新写入思源，安装的技能是真实文件而非目录链接，移动端或无 ChezMoi 环境自动回退内置复制。
...
- 移动端网页可直接输入 `/` 呼出思源原生斜杠菜单并即时筛选（可在模块设置中关闭）。
...
| `lets-document-history-diff` | 文档与单块的 Kramdown 历史差异对比 | 顶栏菜单、块图标菜单、新标签页 |
| `lets-table-fit` | 表格按屏宽自适应，长内容换行 | Damophus 菜单即时开关、设置页 |
| `lets-block-fullwidth` | 图片、表格、数据库、iframe、横向超级块撑满编辑区 | 块图标菜单、文档标题菜单 |
| `lets-style-brush` | 按相同文字批量应用行内格式 | 顶栏菜单、块/文档标题菜单、命令面板 |
| `lets-callout-appearance` | Callout 类型配色、间距、边框与题库标记外观 | 设置页实时预览 |
| `lets-callout-tools` | Callout 智能插入与块菜单批量转换 | 块图标菜单、顶栏菜单、设置页 |
| `lets-mobile-slash-menu` | 移动端 `/` 呼出并筛选原生斜杠菜单 | 自动生效（移动端）、设置页 |
| `lets-exercise-focus` | 按引述块标题识别习题并模糊其余内容 | 命令绑定布局快捷操作 Dock、设置页 |
| `lets-snippet-audit` | 代码片段工作台：识别、预览、启停与编辑 | 插件菜单、独立标签页、设置页 |
| `lets-expanded-plugin-menu` | 展开插件菜单为自适应多列面板并自动发现入口 | 自动生效（块菜单）、设置页 |
...
| 思源 AI 技能管理、ChezMoi 文件同步与内置复制兼容模式 | [技能管理](docs/skill-management.md) |

export const en = {
  "lets-document-format.displayName": "Document format",
  "lets-document-format.description": "Format documents with focused cleanup tools",
  "lets-document-format.enableEmptyParagraphCleanup": "Enable empty text block cleanup",
  "lets-document-format.enableEmptyParagraphCleanupDescription": "Show the command to remove empty text blocks.",
  "lets-document-format.enableSelfReferenceCleanup": "Enable document self reference cleanup",
  "lets-document-format.enableSelfReferenceCleanupDescription": "Show the command to remove references to the current document.",
  "lets-document-format.enableEmptyContainerCleanup": "Enable empty container cleanup",
  "lets-document-format.enableEmptyContainerCleanupDescription": "Show the option to remove empty quote, callout, super block, and list containers.",
  "lets-document-format.enableCodeBlankLineCleanup": "Enable code blank-line cleanup",
  "lets-document-format.enableCodeBlankLineCleanupDescription": "Show the option to remove blank lines inside code blocks.",
  "lets-document-format.menu": "Document format",
  "lets-document-format.removeEmptyParagraphs": "Remove empty text blocks",
  "lets-document-format.commandRemoveEmptyParagraphs": "Remove empty text blocks from the current document",
  "lets-document-format.removeSelfReferences": "Remove document self references",
  "lets-document-format.commandRemoveSelfReferences": "Remove self references from the current document",
  "lets-document-format.noDocument": "Open a document before formatting it.",
  "lets-document-format.openSelectedDocument": "Open the selected document before formatting it.",
  "lets-document-format.noEmptyParagraphs": "No empty text blocks were found.",
  "lets-document-format.confirmTitle": "Remove empty text blocks",
  "lets-document-format.confirmDescription": "Found {count} empty text blocks in this document.",
  "lets-document-format.includeContainers": "Also remove {count} empty text blocks inside containers",
  "lets-document-format.includeEmptyContainers": "Also remove {count} empty containers",
  "lets-document-format.cleanCodeBlankLines": "Also clean blank lines in {count} code blocks",
  "lets-document-format.cancel": "Cancel",
  "lets-document-format.remove": "Remove",
  "lets-document-format.removed": "Removed {count} empty text blocks",
  "lets-document-format.failed": "Could not remove empty text blocks. The document was not changed.",
  "lets-document-format.noSelfReferences": "No references to this document were found.",
  "lets-document-format.confirmSelfReferencesTitle": "Remove document self references",
  "lets-document-format.confirmSelfReferencesDescription": "Remove {count} references from {blocks} text blocks?",
  "lets-document-format.selfReferencesRemoved": "Removed {count} document self references",
  "lets-document-format.selfReferencesFailed": "Could not remove document self references. The document was not changed.",
};

export const zhCN: typeof en = {
  "lets-document-format.displayName": "文档格式化",
  "lets-document-format.description": "提供可逐步扩展的文档整理功能",
  "lets-document-format.enableEmptyParagraphCleanup": "启用空文本块清理",
  "lets-document-format.enableEmptyParagraphCleanupDescription": "显示删除空文本块的功能入口。",
  "lets-document-format.enableSelfReferenceCleanup": "启用文档自身引用清理",
  "lets-document-format.enableSelfReferenceCleanupDescription": "显示清理指向当前文档引用的功能入口。",
  "lets-document-format.enableEmptyContainerCleanup": "启用空容器块清理",
  "lets-document-format.enableEmptyContainerCleanupDescription": "显示删除空引述、Callout、超级块和列表容器的选项。",
  "lets-document-format.enableCodeBlankLineCleanup": "启用代码块空行清理",
  "lets-document-format.enableCodeBlankLineCleanupDescription": "显示删除代码块内部空行的选项。",
  "lets-document-format.menu": "文档格式化",
  "lets-document-format.removeEmptyParagraphs": "删除空文本块",
  "lets-document-format.commandRemoveEmptyParagraphs": "删除当前文档中的空文本块",
  "lets-document-format.removeSelfReferences": "清理文档自身引用",
  "lets-document-format.commandRemoveSelfReferences": "清理当前文档中的自身引用",
  "lets-document-format.noDocument": "请先打开要格式化的文档。",
  "lets-document-format.openSelectedDocument": "请先打开所选文档，再进行格式化。",
  "lets-document-format.noEmptyParagraphs": "未发现空文本块。",
  "lets-document-format.confirmTitle": "删除空文本块",
  "lets-document-format.confirmDescription": "当前文档发现 {count} 个空文本块。",
  "lets-document-format.includeContainers": "同时删除容器块内的 {count} 个空文本块",
  "lets-document-format.includeEmptyContainers": "同时删除 {count} 个空容器块",
  "lets-document-format.cleanCodeBlankLines": "同时清理 {count} 个代码块中的空行",
  "lets-document-format.cancel": "取消",
  "lets-document-format.remove": "删除",
  "lets-document-format.removed": "已删除 {count} 个空文本块",
  "lets-document-format.failed": "无法删除空文本块，文档未被修改。",
  "lets-document-format.noSelfReferences": "未发现指向当前文档的引用。",
  "lets-document-format.confirmSelfReferencesTitle": "清理文档自身引用",
  "lets-document-format.confirmSelfReferencesDescription": "将从 {blocks} 个文本块中清理 {count} 个自身引用，是否继续？",
  "lets-document-format.selfReferencesRemoved": "已清理 {count} 个文档自身引用",
  "lets-document-format.selfReferencesFailed": "无法清理文档自身引用，文档未被修改。",
};

...
Damophus mirrors Topic relations by stable question ID. If question content is deleted and pasted again with the same `custom-qb-id`, scan and confirm the Question Index changes; historical attempts and previously recorded Topic relations reconnect to the recreated block automatically. A different live block with the same ID remains a blocker and is never rebound silently.

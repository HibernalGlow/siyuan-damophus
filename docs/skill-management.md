...
ChezMoi is the default desktop backend. Damophus invokes the configured `chezmoi` command with the configured source directory, the current workspace skill directory, `mode=file`, and explicit source paths for the selected skills. It applies only readable skills that need an update, then runs `chezmoi verify`. ChezMoi metadata remains outside the SiYuan skill directory, and installed skills are real files rather than directory links.

...
- Specialized-Flashcard-Plugin（SFP）是第一阶段的动态汇总和分组管理参考。DAMO 迁移 SQL 分组复习、分类管理、缓存和查询优先、到期卡入口、向上传递识别、自动推迟、批量优先级、原始/过滤后结果查看和文档流入口；不迁移旧的自动统一优先级设置，迁移后由 DAMO 自己维护生命周期、存储、适配器和 UI。
...
标签 SQL 可能同时命中根块和嵌套答案块。原始 SQL 结果视图保留逐行命中以便审计；过滤结果、登记、复习和批量优先级操作只消费去重后的显式卡片根块。

插件菜单和入口遵循工作台模型：闪卡只注册一个顶层菜单项，点击打开闪卡设置工作台，原有复习/分组动作作为子菜单；命令面板、桌面 Dock 和移动 Dock 也只打开该工作台。原生 `siyuan-card` 复习界面继续作为唯一复习界面。
...
| 批量优先级 | 选择 P1-P4 标签后由 DAMO 预览/确认；P1-P4 标签是事实源，运行时数值只是可重建投影 |
...
- **继续调用 Tomato 或文档流私有 API**：拒绝作为核心依赖。优先级只调用 Riff 官方 API；不可用时保留 Markdown 标签并报告 pending。

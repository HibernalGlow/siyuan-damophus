...
同一张卡的优先级标签按卡片根块的最浅 Markdown/Kramdown 层级计算：同层重复的同一标签会去重；同层出现不同的 P1-P4 标签时视为冲突，查找结果可以显示该卡但制卡或批量修改不得静默选择其中一个。嵌套显式卡片拥有自己的标签作用域，子卡标签不会覆盖父卡；没有显式子卡根块时，嵌套标签才作为父卡内容的一部分参与解析。

...
若当前 SiYuan/Riff 没有稳定优先级写入能力，DAMO 仍维护 P1-P4 标签，并在
原生队列钩子中应用排序；标签写入失败时返回 pending。不得依赖 Tomato 私有 API
才报告整个功能成功。

当前版本没有公开且可验证的 Riff 优先级写入 API。DAMO 因此在原生
`Plugin.updateCards()` 边界读取卡片根块标签，按 P1、P2、P3、P4、无标签/冲突
稳定排序；同一优先级保持 Riff 原顺序。可选随机模式从较低优先级候选中抽取约
5%，插入最高优先级区段的前 1/3，但不修改 due、interval、历史或 Markdown
以外的运行时优先级属性。

同一张卡的优先级标签按卡片根块的最浅 Markdown/Kramdown 层级计算：同层重复的同一标签会去重；同层出现不同的 P1-P4 标签时视为冲突，查找结果显示冲突提示，优先级修改不得静默选择其中一个，但不阻止按独立卡片属性制卡。嵌套显式卡片拥有自己的标签作用域，子卡标签不会覆盖父卡；没有显式子卡根块时，嵌套标签才作为父卡内容的一部分参与解析。制卡始终按最近显式卡片根块去重并登记，允许父卡与嵌套子卡同时存在。
...
标签通常同时命中卡片根块和其嵌套答案块。原始 SQL 结果保留这些逐行命中，便于审计查询；过滤结果必须只展示每个显式卡片根块一次，并以根块集合作为登记、复习和批量操作的输入。

闪卡在 DAMO 插件菜单中只占一个顶层入口。入口本身打开闪卡工作台；设置、全部到期复习和启用分组复习作为该入口的子菜单项。命令面板、桌面 Dock 和移动 Dock 同样只打开这个工作台页签，复习内容继续交给思源原生 `siyuan-card`。

插件菜单还可从当前文档或当前笔记本建立临时范围，并选择是否与一个启用的
SQL 分组取交集。工作台保存最近范围、使用次数和置顶状态；这些记录是可删除的
导航历史，不是卡片身份或调度状态。

工作台的闪卡浏览器显示登记卡片的 renderer、P1-P4/冲突、到期状态和命中分组。
原生复习工具栏只增加定位原块、取消登记、P1-P4 和打开工作台；跳过继续使用
思源原生按钮。桌面和移动端共享动作契约，取消登记绝不删除原笔记块。

...
- 在编辑器中实现可复用的虚拟考点关系栏：题目块显示考点，考点笔记锚点显示自身身份；两端分组展示其他同考点笔记和关联题目。
- 具体目标使用思源原生块引用悬浮和跳转；多来源选择由插件管理，移动端使用底部面板，原生悬浮不可用时只挂载一个只读 Protyle 兜底实例。
- 使用当前工作区 SQL 精确解析稳定考点 ID，按可配置路径关键词排序并缓存；DOM 卸载、属性变化和同步完成时失效，不执行任何自动文档写入。
...
- 题目块与考点笔记锚点均能显示虚拟考点关系；其他同考点笔记和关联题目分组正确，原生悬浮链接可用，关系栏不会写入或导出为文档内容。
